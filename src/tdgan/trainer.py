...
class GANTrainer(TowerTrainer):
    ...
    Args:
        ...
        model(tdgan.GAN.GANModelDesc): Model to train.
    ...
    def __init__(self, input, model, d_period=1, g_period=1):
        ...
        with tf.compat.v1.name_scope('optimize'):
            self.d_min = opt.minimize(
                model.d_loss, var_list=model.discrim, name='d_min')
            self.g_min = opt.minimize(
                model.g_loss, var_list=model.gen, name='g_min')
...
class MultiGPUGANTrainer(TowerTrainer):
    ...
    def __init__(self, nr_gpu, input, model):
        ...
        with tf.compat.v1.name_scope('optimize'):
            ...
            g_min = opt.minimize(g_loss, var_list=model.gen,
                                 name='g_op')

            with tf.control_dependencies([g_min]):
                d_min = opt.minimize(d_loss, var_list=model.discrim,
                                     name='d_op')
